...
class ConceptRetrieval:
    ...
    def _filter_and_group_properties(
            cls, property_mapping: Dict[int, Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[int, List[Dict[str, Any]]]]:
        """
        按对象类型分组并筛选属性
    ...
    async def rank_relation_types(cls, query: str, network_details: Dict[str, Any], top_k: int = 10, additional_context: Optional[str] = None,
        ...
        prompt = cls._build_context_and_finalize_prompt(
            prompt, query, additional_context, session_id, objects_mapping, current_kn_id)
