            ...
            if object_types_list:
                for obj_type in object_types_list:
                    ...
                    if obj_type_id and primary_keys:
                        # 保存第一个主键字段名（通常只有一个主键）
                        primary_key_value = primary_keys[0] if isinstance(
                            primary_keys, list) and len(primary_keys) > 0 else None
        ...
        try:
            ...
            if enable_keyword_context:
                ...
                if not object_type_id:
                    raise ValueError(
                        "当enable_keyword_context=True时，object_type_id参数必须提供。因为即使用关键词，但是有很多对象类型，你不知道这个关键词属于哪个对象类型。")
