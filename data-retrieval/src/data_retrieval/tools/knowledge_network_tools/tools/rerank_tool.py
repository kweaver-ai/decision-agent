...
class KnowledgeNetworkRerankTool:
    ...
    def _format_intent_text(cls, intent: List[Dict[str, Any]]) -> str:
        ...
        for idx, intent_item in enumerate(intent):
            ...
            if query_segment:
                ...
                # 添加相关概念信息
                if related_concepts:
                    concept_names = [concept.get("concept_name", concept.get("concept_id", ""))
    ...
    def _build_rerank_prompt(
            cls, question: str, concepts: List[Dict[str, Any]], intents: List[Dict[str, Any]] = None) -> str:
        """构建重排序的提示词
        Args:
            question: 问题文本
            concepts: 概念列表
...
请只返回与问题相关的概念编号，按照相关性排序，以列表形式返回，例如：[3, 1, 5]，都不相关，返回[]"""
    ...
    @classmethod
    async def _call_vector_for_rerank(
            cls, question: str, concepts: List[Dict[str, Any]], batch_size: int = 128) -> List[tuple]:
