...
class SemanticInstanceRetrieval:
    ...
    async def _global_rerank_and_trim_instance_map(
        ...
        *,
        ...
        if not enable_rerank:
            ...
            logger.info(
                f"[向量重排序] enable_rerank=False，使用原始_score分数：{original_score_count}个，降级到关键词匹配：{keyword_score_count}个")
        else:
            # enable_rerank=True 时，使用向量重排序
            scores = await cls._rerank_texts(
                query=query, texts=texts, batch_size=batch_size, enable_rerank=enable_rerank
            )
        ...
        if not enable_rerank:
            ...
            for obj_id, pairs in per_type_scored.items():
                ...
                for idx, (score, inst) in enumerate(pairs[:20], 1):  # 只打印前20个
                    ...
                    display_name = inst_name[:50] if inst_name else 'N/A'
                    logger.info(
                        f"    [{idx}] {display_name}: relevance_score={score}, "
                        f"原始_score字段={original_score}"
                    )
                if len(pairs) > 20:
    ...
    async def _select_fields_for_all_object_types(
        ...
        *,
        ...
        # enable_rerank 参数已从函数参数传入，不再从 semantic_config 中读取
        obj_type_count = len(set(e['object_type_id'] for e in field_entries))
        logger.info(
            f"[向量重排序] 全局属性语义筛选：enable_rerank={enable_rerank}，"
            f"对象类型数量={obj_type_count}，字段数量={len(field_entries)}"
        )
