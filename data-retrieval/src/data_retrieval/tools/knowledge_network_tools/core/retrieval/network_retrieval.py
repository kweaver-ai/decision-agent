...
class KnowledgeNetworkRetrieval:
    ...
    @classmethod
    async def _build_knowledge_network_prompt(
        cls,
        query: str,
        kn_ids: List[str],
        network_details: Dict[str, Dict[str, Any]],
        filtered_relations_by_kn: Dict[str, List[Dict[str, Any]]],
        ...
        if not filtered_relations_by_kn:
            filtered_relations_by_kn = await cls._filter_all_relation_paths_with_rerank(
                query, kn_ids, network_details, session_id,
                top_k_per_network=3, top_k_total=10, enable_rerank=enable_rerank
            )
        ...
        base_instruction = (
            "请根据以下问题、知识网络名称、对象类型和关系类型路径，"
            "返回和问题可能相关的知识网络编号，按相关性排序，不要解释，"
            "以列表形式返回数字编号，例如：[2, 1, 3]，不能返回空列表[]"
        )
        if additional_context:
            prompt += (
                f"{base_instruction.replace('路径，', '路径，以及额外的上下文信息，')}"
    ...
    async def _rank_knowledge_networks(
        ...
        try:
            # 构建提示词
            prompt, kn_mapping = await cls._build_knowledge_network_prompt(
                query, kn_ids, network_details, filtered_relations_by_kn,
                additional_context, session_id
            )

            # 调用LLM并获取结果
            system_message = (
                "你是一个智能知识网络筛选助手。你能够根据用户提出的问题，"
                "从知识网络列表中筛选出相关的知识网络，并按照相关性进行排序，"
                "只返回相关知识网络的编号。"
            )
            error_context = (
                "上一次的回答为空或格式不正确，请重新分析问题并严格按照要求"
                "返回数字编号列表，例如：[2, 1, 3]，"
                "务必选择一个可能相关的知识网络编号，无论如何不能返回空列表[]。"
            )

            content = await LLMClient.call_llm_with_retry(
                prompt, system_message, query, error_context,
