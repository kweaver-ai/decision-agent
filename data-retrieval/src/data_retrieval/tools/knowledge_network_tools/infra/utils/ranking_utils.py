...
class UnifiedRankingUtils:
    ...
    async def _rerank_texts(
        ...
        logger.info(
            f"[向量重排序] 开始调用RerankClient.ado_rerank，texts数量={len(texts)}，batch_size={batch_size}，query长度={len(query)}")
        ...
        for i in range(0, len(texts), max(int(batch_size), 1)):
            ...
            try:
                ...
                logger.debug(
                    f"[向量重排序] RerankClient.ado_rerank返回，batch索引={i}，返回分数数量={len(rerank_scores) if rerank_scores else 0}")
