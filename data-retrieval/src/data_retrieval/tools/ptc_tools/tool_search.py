...
class ToolSearchTool(LLMTool):
    ...
    def _normalize_language(self, language: Optional[str]) -> str:
        ...
        if language not in self._supported_languages:
            logger.warning(
                f"Unsupported language: '{language}'. Supported languages: {self._supported_languages}. Using default language: {self._default_language}")
            return self._default_language
    ...
    def _build_filtered_index(self, tool_scope: Optional[List[str]] = None, language: Optional[str] = None):
        ...
        logger.info(
            f"Building filtered name search index for scope: {tool_scope}, language: {language} ({len(filtered_name_corpus_kv)} tools)")
        ...
        logger.info(
            f"Building filtered description search index for scope: {tool_scope}, language: {language} ({len(filtered_desc_corpus_kv)} tools)")
    ...
    async def _handle_get_usage(
            self, tool_name: str, tool_scope: Optional[List[str]], language: Optional[str] = None) -> dict:
        """处理获取工具文档请求"""
        # 验证并规范化语言参数
        language = self._normalize_language(language)
