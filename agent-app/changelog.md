...
## 1.0.6
- refact: change version package name to appruntime to avoid case-insensitive issues on Windows and Mac Operating systems

## 1.0.7
- feat: 添加Agent-App Run信息的可观测性日志埋点，包括成功和失败的日志上报
- feat: Agent-App 默认使用V2版本Agent-Executor,去除agent-executor-adapter，去除conversation-session-init的逻辑
- feat: 添加uniquery服务依赖，用于查询可观测日志索引库视图数据
- feat: 新增可观测性服务，用于轨迹分析，包含session列表，session详情，run列表，run详情，以及Agent/Session/Run级别的指标查询工具
- feat: chat接口新增chatoption参数，支持 是否开启多轮对话、是否输出progress、是否开启agent配置缓存、是否对文档召回结果进行引用处理
- feat: 更新agent-api.yaml 文件，更新apichatt 接口
- feat: 重构 Session 管理逻辑，支持通过 session service 统一管理会话生命周期
- feat: 增加读取 secret/agent-app-secret.yaml 配置，增加 redis 相关配置
- feat: 数据库SQL脚本下沉到微服务内部管理 && 数据库合并到ADP库
- chore: helm 配置中添加 redis 依赖声明，优化 redis 密码配置安全性

## 1.0.8
- feat: 轨迹分析数据排除api_chat类型的数据调用
