...
import (
	...
	"github.com/kweaver-ai/decision-agent/agent-app/src/domain/service/agentsvc/chatlogrecord"
...
func (agentSvc *agentSvc) handleProgress(ctx context.Context, req *agentreq.ChatReq, progresses []*agentrespvo.Progress) ([]*agentrespvo.Progress, error) {
	...
	if req.InterruptedAssistantMsgID != "" {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "[handleProgress] get interrupted progress err")
		}
		// 初始化 content 以避免 nil
		content := conversationmsgvo.AssistantContent{
		...
		if assistantMsgPO.Content != nil && *assistantMsgPO.Content != "" {
			...
			if err != nil {
				...
				return nil, errors.Wrapf(err, "[handleProgress] unmarshal assistant content error, id: %s, err: %v", req.InterruptedAssistantMsgID, err)
			}
		}
		// 新增：nil 检查，避免 panic
