...
class TestDolphinSDKException:
    ...
    def test_dolphin_sdk_model_exception(self, mock_debug):
        """测试 Dolphin SDK 模型异常"""
        raw_exc = ModelException("Model error")
        ...
        assert "ModelExecption" in str(exc.error.error_code)

    @patch("app.common.config.Config.is_debug_mode", return_value=False)
    def test_dolphin_sdk_skill_exception(self, mock_debug):
        """测试 Dolphin SDK 技能异常"""
        raw_exc = SkillException("Skill error")
        exc = DolphinSDKException(
            raw_exception=raw_exc,
            agent_id="agent123",
            session_id="session456",
        ...
        assert "SkillExecption" in str(exc.error.error_code)

    @patch("app.common.config.Config.is_debug_mode", return_value=False)
    def test_dolphin_sdk_base_exception(self, mock_debug):
        """测试 Dolphin SDK 基础异常"""
        raw_exc = Exception("Generic error")
        exc = DolphinSDKException(
            raw_exception=raw_exc,
            agent_id="agent123",
            session_id="session456",
        ...
        assert "BaseExecption" in str(exc.error.error_code)
