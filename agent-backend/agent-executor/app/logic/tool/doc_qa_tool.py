...
from app.driven.dip.data_connection_service import DpDataSourceService
    ...
    async def single_doc_rag(props, context):
        ...
        for doc in props["data_source"]["doc"]:
            if doc.get("ds_id") and doc["ds_id"] != "0":
                ...
                data_connection_service = DpDataSourceService(headers)
                ds_info = await data_connection_service.get_datasource_by_id(ds_id)
                ...
                props["headers"]["as-token"] = await oauth_service.get_token(
                    ...
                    await data_connection_service.decode_password(
