...
def init_trace_provider(server_info: ServerInfo, setting: TraceSetting) -> None:
    ...
    """
    try:
        print("[OTel] init_trace_provider called")
        print("[OTel]   server_name={server_info.server_name}")
        print("[OTel]   server_version={server_info.server_version}")
        print("[OTel]   trace_provider={setting.trace_provider}")
        print("[OTel]   otlp_endpoint={setting.otlp_endpoint}")
        ...
        if setting.trace_provider == "console":
            ...
            print("[OTel] Created ConsoleSpanExporter")
            ...
            print("[OTel] ✅ OTLPSpanExporter created successfully")
        ...
        print("[OTel] Creating BatchSpanProcessor...")
        ...
        print("[OTel] Building resource attributes...")
        resource_attributes = {
        ...
        # 创建 TracerProvider
        print("[OTel] Creating TracerProvider...")
        trace_provider = TracerProvider(
            resource=resource, active_span_processor=trace_processor, sampler=sampler
        )

        print("[OTel] Setting global tracer provider...")
        set_tracer_provider(trace_provider)
        print("[OTel] ✅ Trace provider initialized successfully!")
        print(
