...
def init_observability(server_info: ServerInfo, setting: ObservabilitySetting):
    """初始化可观测性组件"""
    print(f"[Observability] Initializing observability: log_enabled={setting.log.log_enabled}, trace_enabled={setting.trace.trace_enabled}")
    
    if setting.log.log_enabled:
        init_log_provider(server_info, setting.log)

    if setting.trace.trace_enabled:
        print(f"[Observability] Trace enabled, initializing trace provider: provider={setting.trace.trace_provider}, endpoint={setting.trace.otlp_endpoint}")
        init_trace_provider(server_info, setting.trace)
    else:
        print(f"[Observability] Trace is disabled (trace_enabled={setting.trace.trace_enabled})")
