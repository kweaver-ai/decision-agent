...
type IV2AgentExecutor interface {
	...
	Resume(ctx context.Context, req *v2agentexecutordto.V2AgentResumeReq) (chan string, chan error, error)
	// Terminate 终止 Agent 执行
	Terminate(ctx context.Context, req *v2agentexecutordto.V2AgentTerminateReq) error
}
