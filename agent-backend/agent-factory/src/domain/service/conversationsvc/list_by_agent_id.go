...
func (svc *conversationSvc) ListByAgentID(ctx context.Context, agentID, title string, page, size int, startTime, endTime int64) (conversationList []conversationresp.ConversationDetail, count int64, err error) {
	...
	if err != nil {
		otelHelper.Errorf(ctx, "[ListByAgentID] get conversation list by agentID error, agentID: %s, err: %v", agentID, err)
	...
	if err != nil {
		otelHelper.Errorf(ctx, "[ListByAgentID] convert PO to EO error, agentID: %s, err: %v", agentID, err)
	...
	for i, eo := range eos {
		...
		if err != nil {
			otelHelper.Errorf(ctx, "[ListByAgentID] convert EO to DTO error, agentID: %s, err: %v", agentID, err)
