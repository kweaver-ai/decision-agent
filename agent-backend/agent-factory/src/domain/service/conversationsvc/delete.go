...
func (svc *conversationSvc) Delete(ctx context.Context, id string) (err error) {
	...
	if err != nil {
		if chelper.IsSqlNotFound(err) {
			...
			otelHelper.Errorf(ctx, "[Delete] conversation not found, id: %s", id)
			err = capierr.NewCustom404Err(ctx, apierr.ConversationNotFound, "数据智能体配置不存在")

			return
		}

		otelHelper.Errorf(ctx, "[Delete] get conversation by id error, id: %s, err: %v", id, err)
	...
	if err != nil {
		otelHelper.Errorf(ctx, "[Delete] begin tx error, id: %s, err: %v", id, err)
		return
	}
	...
	if err != nil {
		otelHelper.Errorf(ctx, "[Delete] delete conversation error, id: %s, err: %v", id, err)
	...
	if err != nil {
		otelHelper.Errorf(ctx, "[Delete] delete conversation msg error, id: %s, err: %v", id, err)
...
func (svc *conversationSvc) DeleteByAppKey(ctx context.Context, appKey string) (err error) {
	...
	tx, err := svc.conversationRepo.BeginTx(ctx)
	if err != nil {
		otelHelper.Errorf(ctx, "[DeleteByAppKey] begin tx error, appKey: %s, err: %v", appKey, err)
		return
	}
	...
	if err != nil {
		otelHelper.Errorf(ctx, "[DeleteByAppKey] delete conversation error, appKey: %s, err: %v", appKey, err)
		return errors.Wrapf(err, "删除对话数据失败")
	}

	err = svc.conversationMsgRepo.DeleteByAPPKey(ctx, tx, appKey)
	if err != nil {
		otelHelper.Errorf(ctx, "[DeleteByAppKey] delete conversation msg error, appKey: %s, err: %v", appKey, err)
