...
async def handle_upsert(
    ...
    if cache_entity:
        ...
        get_otel_logger().info(f"Agent缓存已存在，更新缓存数据: cache_id={cache_id_vo}")
        ...
        if cache_entity is None:
            # 缓存在更新过程中过期，创建新缓存
            get_otel_logger().warn(f"Agent缓存在更新过程中过期: cache_id={cache_id_vo}")
    ...
    get_otel_logger().info(f"Agent缓存不存在，创建新Cache: agent_id={agent_id}")
