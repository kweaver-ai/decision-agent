...
async def ado_llm_ad(question, content):
    ...
    while 1:
        ...
        # llm_model_api = "https://10.4.30.214:8444/api/model-factory/v1/prompt-template-run"
        # headers = {'Content-Type': 'application/json', 'appid': 'Nr9Sb40T3HqpedSPVnl'}

        llm_model_api = (
            "https://10.4.70.92:8444/api/model-factory/v1/prompt-template-run"
        )
...
"""
    ...
    llm_model_api = "http://192.168.152.11:8303/v1/chat/completions"
...
"""
    ...
    llm_model_api = "http://192.168.216.28:8303/v1/chat/completions"
    headers = {"Content-Type": "application/json"}
    response = requests.post(llm_model_api, headers=headers, json=payload, verify=False)
    response = response.json()
    return response["choices"][0]["message"]["content"].strip()
